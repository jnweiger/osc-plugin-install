...
@cmdln.option('-k', '--keeprepos', metavar='REPOS', help='Specify a list of whitespace seperated repositories. This list decides if a repo used during installation is temporary or kept enabled permanently. Use -k "" for keep-none or -k "+" for keep-all. Default: prompt the user.')
...
def do_install(self, subcmd, opts, *args):
    ...
    osc in 
        take PROJECT name and SRC_PACKAGE name from current directory.
    ...
    osc in PROJECT[/SRC_PACKAGE] PACKAGE
    osc in PROJECT/SRC_PACKAGE
        install PACKAGE from PROJECT. An optional SRC_PACKAGE name may be appended to PROJECT
        for the case that unpublished binaries shall be installed. In this case the SRC_PACKAGE 
        is needed. 
    ...
    If you specify a SRC_PACKAGE instead, osc install will choose the binary package of the same name, 
    or the one with a name that is most similar.
    ...
    repos_to_add = []
    repos_to_enable = []
    known_repos = self._pipe_from_cmd_stdout(('zypper', 'lr', '--export', '-'))     # no sudo with lr
...
def _short_repo_name(self, url):
    url = re.sub('^\w+://', '', url)
    url = re.sub('\bdownload\.', 'd.', url)
    url = re.sub('\bopensuse\.org\b', 'o.o', url)
    url = re.sub('\bpmbs.links2linux.de\b', 'pmbs', url)
    url = re.sub('\bpmbs.links2linux.de\b', 'pmbs', url)
    url = re.sub('\bopenSUSE_1', '1', url)
    return url

def _need_temp_repo(self, url):
    """find out what to do with repo url.
       If it is already in self.known_repos, we put it in self.repos_to_enable and return False.
       If not we need to decide if this is a temporary or permanent repo.
       If opts.keeprepos is there, then we use it with _match_repo_url().
       Otherwise we prompt the user. Note that a user can say 'A' for all. Which then
       sets opts.keeprepos to = '+', so that no further questions will be asked.
...
def _best_platform(self, etc_suse_release, repos, opts):
    ...
    default_platform = 'openSUSE_12.3'
...
def _pipe_from_cmd_stdout(self, cmdv, progress='.', term='\n', tee=False):
  ...
  ## CAUTION: repeated sudo commands all ask again for password. 
  ## If needed, sudo should be called before coming here.
